...
// Fiets Inruil Calculator – app.js (zoals het was; lokale bronnen)
...
async function pasteFromClipboard() {
  const input = document.getElementById('licCodeInput');
  const msg = document.getElementById('licMsg');
  try { const text = await navigator.clipboard.readText(); input.value = (text || '').trim(); msg.textContent = 'Code geplakt'; }
  catch { msg.textContent = 'Plakken wordt door de browser geblokkeerd. Plak handmatig (lang indrukken) of typ de code.'; }
}
...
(function installButtonManager(){
  ...
  installBtn?.addEventListener('click', async () => { if (!deferredPrompt) return; try { await deferredPrompt.prompt(); await deferredPrompt.userChoice; } catch {} deferredPrompt = null; updateInstallVisibility(); });
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  const banner= document.getElementById('licenseBanner');
  const footer= document.getElementById('licenseFooter');
  if (!lic) { if (gateEl) gateEl.hidden = false; if (banner) { banner.textContent = 'Licentie vereist voor gebruik.'; banner.style.display = 'block'; } if (footer) footer.textContent = ''; }
  else { closeLicenseGate(); }
  bindLicenseUI();
  await initData();
});
