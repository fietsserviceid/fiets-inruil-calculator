...
function setOfferKmVisibility(isElectricType, km) { 
 ...
 try { 
 offerKmEl.textContent = Number(km).toLocaleString('nl-NL') + ' km'; 
 } catch { 
 offerKmEl.textContent = String(km); 
 } 
 } 
 } else { 
...
// ====== NIEUW: server-gestuurde validatie en her-validatie ====== 
...
async function revalidateLocalLicenseAgainstServer() { 
 ...
 if (!lic) return false; 
 try { 
 // cache-busting om CDN/browser-caching te omzeilen 
 const list = await fetchJSON(`${CODES_SOURCE}?t=${Date.now()}`); 
 const entry = (list.codes ?? []).find((c) => c.code === lic.code); 
 // Niet (meer) geldig? -> direct intrekken 
 if (!isServerLicenseValid(entry)) { 
 localStorage.removeItem(LIC_STORAGE_KEY); 
 return false; 
 } 
 // Server 'valid_until' kan lokale expiry inkorten 
...
async function activateLicenseFromInput() { 
 ...
 new Date(newLic.expiresAt).toLocaleDateString('nl-NL'); 
 closeLicenseGate(); 
 } catch (e) { 
...
async function pasteFromClipboard() { 
 ...
 msg.textContent = 
 'Plakken wordt door de browser geblokkeerd. Plak handmatig (lang indrukken) of typ de code.'; 
 } 
} 
...
function recalc() { 
 ...
 const kmFactor = isElectricType 
 ? getKmStandFactor(km, DATA.km_stand_factors ?? []) 
 : 1.0; 
 ...
 // (optioneel: velden voor offerte) 
 document.getElementById('resultCard').hidden = false; 
 // v9.6 - extra offertevelden incl. km-stand 
...
async function initData() { 
 ...
 hasAccuSel.addEventListener('change', () => { 
 updateTypeDependent(); 
 try { 
 recalc(); 
 } catch (e) {} 
 ...
 try { 
 document.getElementById('kmStandInput')?.addEventListener('input', recalc); 
 } catch (e) {} 
 ...
 // --- NIEUW: geldigheidsregel op de pagina ---
 const validityEl = document.getElementById('offerValidity');
 if (validityEl) {
   validityEl.textContent = 'Deze offerte is 14 dagen geldig vanaf de printdatum.';
 }
...
(function installButtonManager() { 
 ...
 if (isIOS) { 
 if (banner) { 
 banner.innerHTML = 'Op iPhone: tik Deel ▸ Zet op beginscherm.'; 
...
window.addEventListener('DOMContentLoaded', async () => { 
 ...
 // Licentie overlay tonen of verbergen met her-validatie 
 const lic = getLocalLicense(); 
 const gateEl = document.getElementById('licenseGate'); 
 const banner = document.getElementById('licenseBanner'); 
 const footer = document.getElementById('licenseFooter'); 
 ...
 if (banner) { 
 banner.textContent = 'Licentie vereist voor gebruik.'; 
 banner.style.display = 'block'; 
 } 
 if (footer) footer.textContent = ''; 
 } else { 
 ...
 if (gateEl) gateEl.hidden = false; 
 if (banner) { 
 banner.textContent = 'Licentie ongeldig of verlopen.'; 
 banner.style.display = 'block'; 
 } 
 if (footer) footer.textContent = ''; 
 } 
...
function downloadOffertePDF() { 
 ...
 if (!jsPDF) { 
 alert('PDF-module is nog niet geladen. Probeer nogmaals.'); 
 return; 
 } 
 ...
 doc.setFontSize(16); 
 doc.text('Klant & Fietsgegevens', 20, 35); 
 ...
 // --- NIEUW: Dealer uit de pagina halen ---
 ...
 doc.setFontSize(10); 
 doc.text( 
 'Deze inruilwaarde is gebaseerd op type fiets, leeftijd, staat, km-stand en overige factoren.', 
 20, 
 y 
 ); 
 y += 10;
 // --- NIEUW: formele geldigheidsregel ---
 doc.text('Deze offerte is 14 dagen geldig vanaf de printdatum.', 20, y);
 y += 20; 
 doc.setFontSize(12); 
 doc.text('Handtekening klant:', 20, y); 
 doc.setLineWidth(0.2); 
 doc.line(70, y + 1, 150, y + 1); 
 y += 20; 
 doc.text('Handtekening dealer:', 20, y); 
 doc.setLineWidth(0.2); 
 doc.line(70, y + 1, 150, y + 1); 
 y += 20; 
 doc.text('Datum:', 20, y); 
 doc.text(new Date().toLocaleDateString('nl-NL'), 40, y); 
 doc.save('Offerte-FietsServiceID.pdf'); 
} 
(function () { 
 function bind() { 
 const b = document.getElementById('downloadPdfBtn'); 
 if (b && !b.dataset.pdfBound) { 
 b.addEventListener('click', downloadOffertePDF); 
 b.dataset.pdfBound = '1'; 
 return true; 
 } 
 return false; 
 } 
