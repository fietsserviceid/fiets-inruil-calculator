...
const CODES_SOURCE = './codes.json'; // of zet hier je RAW-URL neer

// ====== Helpers ======
const € = (v) => new Intl.NumberFormat('nl-NL', {style:'currency', currency:'EUR'}).format(v);
...
async function activateLicenseFromInput() {
  ...
  try {
    ...
    closeLicenseGate(list.price_eur_per_year);
...
function closeLicenseGate(price) {
  ...
  if (footer) footer.textContent = 'Licentie actief. Tarief €' + (price ?? 99).toFixed(2) + ' p/j.';
}
...
function recalc() {
  ...
  document.getElementById('offerTotal').textContent = €(value);
...
async function initData() {
  ...
  function updateTypeDependent() {
    ...
    refHint.textContent = 'Referentieprijs voor ' + typeSel.value + ': ' + €(t?.ref_price || 0);
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!lic) {
    ...
    footer.textContent = 'Geen geldige licentie gevonden op dit apparaat.';
  } else {
    closeLicenseGate();
  }
