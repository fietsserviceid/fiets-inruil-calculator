// Fiets Inruil Calculator – app.js (remtype uit data.json + bestaande fixes)
...
function setOfferKmVisibility(isElectricType, km) {
  try {
    ...
    if (!offerKmEl) return;
    const offerKmLabelEl = offerKmEl.previousElementSibling; // Km-stand:
    if (isElectricType) {
...
function saveLocalLicense(licCode, months = 12) {
  const now = new Date();
  const expires = new Date(now); expires.setMonth(expires.getMonth() + months);
  ...
  try {
    ...
    if (entry?.valid_until) {
      ...
      if (!Number.isNaN(+serverExp)) {
        ...
        if (serverExp < localExp) { local.expiresAt = serverExp.toISOString(); localStorage.setItem(LIC_STORAGE_KEY, JSON.stringify(local)); }
      }
    }
...
function isRemtypeApplicable(typeName, cfg) {
  if (!cfg) return false;
  const list = Array.isArray(cfg.applicable_types) ? cfg.applicable_types : [];
  const t = (typeName || '').toLowerCase();
  return list.some((x) => (x || '').toString().toLowerCase() === t);
}

function getRemtypeFactor(typeName, brakeSel, cfg) {
  if (!cfg || !cfg.factors) return 1.0;
  if (!isRemtypeApplicable(typeName, cfg)) return 1.0;
  const v = (brakeSel?.value || '').toLowerCase();
  const factors = cfg.factors;
  const keys = Object.keys(factors);
  if (!v) return 1.0;
  const foundKey = keys.find((k) => k.toLowerCase() === v);
  return foundKey ? Number(factors[foundKey] ?? 1.0) : 1.0;
}
...
function recalc() {
  ...
  const brakeSel = document.getElementById('brakeTypeSelect');
  const brakeCfg = DATA.remtype || null;
  ...
  const remFactor = getRemtypeFactor(typeName, brakeSel, brakeCfg);

  let value = refPrice * ageFactor * stateFactor * brandFactor * accuFactor * kmFactor * remFactor;
  ...
  const kmEl = document.getElementById('factorKmStand'); if (kmEl) kmEl.textContent = kmFactor.toFixed(2);
...
async function initData() {
  ...
  const brakeWrap = document.getElementById('brakeTypeWrap');
  const brakeSel = document.getElementById('brakeTypeSelect');
  ...
  // Remtype opties voeden uit data.json, indien aanwezig
  const brakeCfg = DATA.remtype || null;
  if (brakeCfg?.factors && brakeSel) {
    const opts = Object.keys(brakeCfg.factors).map((k) => ({ value: k, label: k.charAt(0).toUpperCase() + k.slice(1) }));
    populateSelect(brakeSel, opts);
  }
  ...
  function updateTypeDependent() {
    ...
    if (brakeWrap) {
      const showRem = isRemtypeApplicable(typeSel.value, brakeCfg);
      brakeWrap.style.display = showRem ? 'block' : 'none';
      if (showRem && brakeSel && !brakeSel.value) {
        const firstKey = (brakeCfg && brakeCfg.factors) ? Object.keys(brakeCfg.factors)[0] : 'hydraulisch';
        brakeSel.value = firstKey || 'hydraulisch';
      }
    }
  }
  ...
  brakeSel?.addEventListener('change', () => { try { recalc(); } catch(e) {} });

  updateTypeDependent();
  document.getElementById('calcBtn').addEventListener('click', recalc);
  try { document.getElementById('kmStandInput')?.addEventListener('input', recalc); } catch {}
...
(function installButtonManager() {
  ...
  async function detectInstalledRelatedApps() { try { if (!navigator.getInstalledRelatedApps) return false; const related = await navigator.getInstalledRelatedApps(); return Array.isArray(related) && related.length > 0; } catch { return false; } }
  async function updateInstallVisibility() {
    if (isStandalone()) { try { localStorage.setItem('pwaInstalled', '1'); } catch {} hideInstall(); return; }
    if (localStorage.getItem('pwaInstalled') === '1') { hideInstall(); return; }
    if (await detectInstalledRelatedApps()) { try { localStorage.setItem('pwaInstalled', '1'); } catch {} hideInstall(); return; }
    if (isIOS) { if (banner) { banner.innerHTML = 'Op iPhone: tik Deel ▸ Zet op beginscherm.'; banner.style.display = 'block'; } hideInstall(); return; }
  ...
  installBtn?.addEventListener('click', async () => { if (!deferredPrompt) return; try { await deferredPrompt.prompt(); const choice = await deferredPrompt.userChoice; if (choice && choice.outcome === 'accepted') { try { localStorage.setItem('pwaInstalled', '1'); } catch {} } } catch {} deferredPrompt = null; await updateInstallVisibility(); });
  window.addEventListener('appinstalled', async () => { try { localStorage.setItem('pwaInstalled', '1'); } catch {} deferredPrompt = null; await updateInstallVisibility(); });
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    if (priceHeader) priceHeader.textContent = price; if (priceBody) priceBody.textContent = price;
  } catch {}
...
function downloadOffertePDF() {
  ...
  doc.text('Offerte – Inruilvoorstel', 20, 20); doc.setDrawColor(...blue); doc.setLineWidth(0.6); doc.line(20, 25, 190, 25);
  doc.setFontSize(16); doc.text('Klant & Fietsgegevens', 20, 35); doc.setLineWidth(0.25); doc.line(20, 37, 90, 37);
  ...
  let dateTxt = '';
  try { const dEl = document.getElementById('offerDate'); const pageDate = (dEl && typeof dEl.textContent === 'string') ? dEl.textContent.trim() : ''; dateTxt = pageDate || new Date().toLocaleDateString('nl-NL'); } catch { dateTxt = new Date().toLocaleDateString('nl-NL'); }
