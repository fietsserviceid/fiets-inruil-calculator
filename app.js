...
async function activateLicenseFromInput() {
  ...
  try {
    ...
    const found = (list.codes || []).find(
      (c) => c.code === code && c.active !== false
    );
    ...
    msg.textContent =
      ...
      new Date(saved.expiresAt).toLocaleDateString('nl-NL');
...
function recalc() {
  ...
  const age = Math.min(
    15,
    Math.max(0, parseInt(ageInput.value || '0', 10))
  );

  const restTab = hasAccu
    ? DATA.restwaarde.metaccu
    : DATA.restwaarde.zonderaccu;

  const ageFactor = Number(restTab[String(age)] || 0);
  const stateFactor = Number(DATA.cond_factors[stateSel.value] || 1);
  const brandFactor = Number(
    (DATA.brands[typeName] || {})[brandSel.value] || 1
  );
  const accuFactor = hasAccu
    ? Number(DATA.accu_state_factors[accuStateSel.value] || 1)
    : 1;
  ...
  document.getElementById('factorState').textContent =
    stateFactor.toFixed(2);
  document.getElementById('factorAccu').textContent =
    accuFactor.toFixed(2);
  document.getElementById('factorBrand').textContent =
    brandFactor.toFixed(2);
  ...
  document.getElementById('offerDate').textContent =
...
async function initData() {
  ...
  populateSelect(
    accuStateSel,
    Object.keys(DATA.accu_state_factors)
  );
  ...
  function updateTypeDependent() {
    ...
    const brandsForType = Object.keys(
      DATA.brands[typeSel.value] || { Overig: 1 }
    );
...
function bindLicenseUI() {
  const actBtn = document.getElementById('licActivateBtn');
  const pasteBtn = document.getElementById('licPasteBtn');
  const input = document.getElementById('licCodeInput');
...
(function iosViewportPolyfill() {
  const supportsDvh =
    CSS && CSS.supports && CSS.supports('height', '100dvh');
...
(function installButtonManager() {
  ...
  installBtn?.addEventListener('click', async () => {
    if (!deferredPrompt) {
      return; // iOS — hier kun je een iOS “Zet op beginscherm”-overlay tonen
    }
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const priceHeader =
      document.getElementById('licensePriceHeader');
  ...
  // Bind licentie‑UI (plakken/activeer)
  bindLicenseUI();
