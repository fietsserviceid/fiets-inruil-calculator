...
async function fetchJSON(url) {
  // Forceer verse versie (om SW/PWA caching problemen te vermijden)
...
async function pasteFromClipboard() {
  try {
    ...
    document.getElementById('licCodeInput').value = (text || '').trim();
  } catch {
    // negeer
  }
}
...
function recalc() {
  ...
  const refPrice = (parseFloat(priceInput.value) > 0)
...
// ====== iOS overlay viewport fix ======
// Voorkomt dat fullscreen overlay (licentie) op iPhone “afgesneden” wordt.
// Werkt samen met CSS: .license-overlay { height: 100dvh; min-height: -webkit-fill-available; }
...
// Verbergt de knop als app al geïnstalleerd/standalone draait; toont alleen wanneer zinvol.
// (Vervangt oude, eenvoudige beforeinstallprompt-handler.)
(function installButtonManager(){
  ...
  function updateInstallVisibility() {
    ...
    // Android/Chrome/Edge → toon alleen als prompt beschikbaar is
  ...
  installBtn?.addEventListener('click', async () => {
    if (!deferredPrompt) {
      // iOS: hier kun je optioneel een overlay tonen met "Zet op beginscherm".
      return;
    }
    try {
...
window.addEventListener('DOMContentLoaded', async () => {
  try {
    // Toon prijsindicatie in licentie overlay
    const list  = await fetchLicenseList();
    ...
    // negeer; UI blijft werken
  }

  // Licentie‑gate openen/sluiten
